 ...
 *
 * Adapted for non-paludis usage by Fernando J. Pereda <fpereda@gmail.com>
 */
...
SimpleParserExpression::SimpleParserExpression(
		const SimpleParserMatchFunction & f) :
    _match(f)
{
}

std::string::size_type
SimpleParserExpression::match(const std::string & s,
		const std::string::size_type offset) const
{
...
{
    std::string::size_type
    m_and(const SimpleParserExpression & e1,
			const SimpleParserExpression & e2, const std::string & text,
    ...
    std::string::size_type
    m_capture(const SimpleParserExpression & e1, std::string & var,
			const std::string & text, const std::string::size_type offset)
    {
        std::string::size_type len_1(e1.match(text, offset));
        if (std::string::npos != len_1)
    ...
    int compare_ignoring_case(const std::string & text,
			const std::string::size_type offset,
			const std::string::size_type length, const std::string & pattern)
    {
            ...
            const std::string::size_type offset, const bool ignore_case)
    {
        ...
        if (! ignore_case)
            return (0 == text.compare(offset, pattern.length(), pattern))
				? pattern.length() : std::string::npos;
        else
            return (0 == compare_ignoring_case(
						text, offset, pattern.length(), pattern))
				? pattern.length() : std::string::npos;
    }
...
simple_parser::operator& (const SimpleParserExpression & e1,
		const SimpleParserExpression & e2)
{
...
{
    using namespace std::placeholders;
    return SimpleParserExpression(
			std::bind(m_capture, e1, std::ref(var), _1, _2));
}
...
SimpleParserExpression
simple_parser::exact(const std::string & s)
{
    using namespace std::placeholders;
...
SimpleParserExpression
simple_parser::exact_ignoring_case(const std::string & s)
{
    using namespace std::placeholders;
...
SimpleParserExpression
simple_parser::any_of(const std::string & s)
{
    using namespace std::placeholders;
...
SimpleParserExpression
simple_parser::any_except(const std::string & s)
{
    using namespace std::placeholders;
...
{
    std::string::size_type len_1(e1.match(_text, _offset));
    if (std::string::npos == len_1)
        return false;
    else
    {
...
const std::string
SimpleParser::rest() const
{
	return _text.substr(_offset);
}
...
SimpleParser::current_line_number() const
{
    ...
    for (std::string::size_type p(0) ; p < _offset ; ++p)
        if (_text[p] == '\n')
            ++line_number;
